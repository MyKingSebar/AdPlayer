...
public class LogUploadManager {
	...
	private static LogUploadManager INSTANCE = null;
	...
	private LogUploadManager() {
		mHandlerThread = new HandlerThread("logmgr_thread");
	...
	private String getTempLogFilePath(final int logmodule, final String deviceid, final String time) {
		StringBuilder sb = new StringBuilder();
	...
	private void removeLogThreadInfoFromList(String file, LogTaskInfo taskinfo) {
		if (taskinfo == null) {
			mLogger.i("The taskinfo is null.");
			return;
		}
		if (taskinfo.logthdinfolist == null) {
			mLogger.i("The taskinfo.logthdinfolist is null.");
			return;
		}
		...
		for (LogThreadInfo thdinfo : taskinfo.logthdinfolist) {
			if (thdinfo.file.equals(file)) {
				if (!taskinfo.logthdinfolist.remove(thdinfo)) {
					mLogger.i("Failed to remove the thread info: "+thdinfo);
				}
				return;
			}
		}
	...
	private void uploadFile(final String file, final LogTaskInfo info) {
		if (!FileUtils.isExist(file)) {
			mLogger.i("The local file isn't exist.");
			return;
		}
        ...
        FtpHelper.getInstance().uploadFileList(toUploadList, new FtpOperationInterface() {
			...
			public void aborted(String file) {
				mLogger.i("Aborted to upload the file: '"+file+"'.");
			...
			public void completed(String file) {
				mLogger.i("Completed to upload the file: '"+file+"'.");
				FileUtils.delFile(file);
				clearTaskInfoList(file, info);
			}
			...
			public void failed(String file) {
				mLogger.i("Failed to upload the file: '"+file+"'.");
				FileUtils.delFile(file);
				clearTaskInfoList(file, info);
			}
        });
		...
		synchronized (mLogTaskList) {
			if ((mLogTaskList != null) && mLogTaskList.contains(info)) {
				if (!mLogTaskList.remove(info)) {
					mLogger.i("Failed to remove the task info: "+info);
				}
			}
		}
	}
	...
	private void doUploadLogForNormal(final LogTaskInfo info, final int logmodule,
			final String deviceid, final String time) {
		final String tmpFile = getTempLogFilePath(logmodule, deviceid, time);
	...
	private void uploadLogForNormal(final LogTaskInfo info) {
		...
		String deviceid = SysParamManager.getInstance().getDeviceId();
		String time = getCurrentTime();

		doUploadLogForNormal(info, LOGMODULE_MAIN, deviceid, time);
		doUploadLogForNormal(info, LOGMODULE_SYSTEM, deviceid, time);
		doUploadLogForNormal(info, LOGMODULE_EVENTS, deviceid, time);
	}
	...
	private void uploadLogForPeriod(final LogTaskInfo info) {
		...
		tmpFile = getTempLogFilePath(LOGMODULE_MAIN, deviceid, time);
		...
		tmpFile = getTempLogFilePath(LOGMODULE_SYSTEM, deviceid, time);
		...
		tmpFile = getTempLogFilePath(LOGMODULE_EVENTS, deviceid, time);
				...
				while (!isInterrupted()) {
					...
					if (info != null) {
						...
						case LOGTYPE_PERIOD:
							...
							break;
						default:
							mLogger.e("The type is invalid. Just removes it.");
	...
	public synchronized void uploadLog(int type, long millisec) {
		if ((type != LOGTYPE_NORMAL) && (type != LOGTYPE_PERIOD)) {
			mLogger.e("The type is invalid, type = "+type+".");
			return;
		}
		if ((mLogTaskList != null) && (mLogTaskList.size() > DEFAULT_MAX_LOG_TASK_NUM)) {
			mLogger.i("The task number has been the max value.");
			return;
		}
