...
public class AdApplication extends Application {
	...
	public String getStandbyImagePath() {
		...
		sb.append(File.separator);
		sb.append(Constants.STANDBYDIRNAME);
		sb.append(File.separator);
	...
	public boolean releaseMaterialSpace(long length) {
		if (length <= 0) {
			mLogger.i("Length is zore. No need to release.");
			return true;
		}

		List<FileInfo> filelst = FileUtils.getFileInfo(getApplicationDir(), true);
		if (filelst == null) {
			mLogger.i("File info list is null.");
			return false;
		} else if (filelst.isEmpty()) {
			mLogger.i("No file info in the list.");
			return false;
		}

		// Sort file info
		FileInfo[] files = new FileInfo[filelst.size()];
		files = filelst.toArray(files);
		...
		for (FileInfo info : files) {
			FileUtils.delFile(info.path);
			releaselength += info.length;
			if (releaselength >= length) {
				return true;
			}
		}
